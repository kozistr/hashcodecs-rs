    ...
    Chart(
        ...
        (
            panel(
                'Standard encode',
                SIZES,
                hashcodecs=[5.35, 23.57, 3.27, 4.65],
            ...
            panel(
                'Standard decode',
                SIZES,
                hashcodecs=[4.15, 17.94, 4.07, 4.12],
            ...
            panel(
                'URL-safe encode',
                SIZES,
                hashcodecs=[5.11, 22.60, 3.33, 4.66],
            ...
            panel(
                'URL-safe decode',
                SIZES,
                hashcodecs=[7.30, 14.09, 4.16, 4.36],
    ...
    Chart(
        ...
        tuple(
            ...
            for title, ours, upstream in (
                ('x86 32-bit one-shot', [3.54, 3.86, 3.99, 3.99], [3.44, 3.72, 3.84, 3.83]),
                ('x86 32-bit incremental', [2.84, 3.62, 4.00, 3.99], [2.86, 3.48, 3.84, 3.84]),
                ('x86 128-bit one-shot', [6.61, 8.59, 9.46, 9.45], [6.69, 8.22, 8.89, 8.86]),
                ('x86 128-bit incremental', [4.40, 7.20, 9.40, 9.41], [0.69, 0.78, 0.80, 0.81]),
                ('x64 128-bit one-shot', [6.89, 8.96, 10.07, 10.09], [7.79, 9.48, 10.26, 10.26]),
    ...
    Chart(
        ...
        (
            ...
            panel(
                'XXH3-64 batch (32 items)',
                ['64 B', '1 KiB', '4 KiB', '1 MiB'],
                hashcodecs=[5.18, 43.70, 62.21, 36.85],
                xxhash=[2.15, 14.55, 30.50, 18.39],
            ),
            panel(
                'XXH3-128 batch (32 items)',
                ['64 B', '1 KiB', '4 KiB', '1 MiB'],
                hashcodecs=[2.72, 28.89, 52.73, 36.50],
    ...
    Chart(
        ...
        tuple(
            ...
            for title, ours, loop, pybase64, cpython in (
                ('16 B encode', [0.55, 0.72, 0.75], [0.30, 0.33, 0.33], [0.12, 0.13, 0.13], [0.18, 0.19, 0.19]),
                ('16 B decode', [0.46, 0.62, 0.61], [0.22, 0.24, 0.24], [0.08, 0.08, 0.08], [0.12, 0.12, 0.13]),
                ('256 B encode', [7.03, 8.80, 8.50], [4.33, 4.41, 4.34], [1.86, 1.82, 1.87], [0.38, 0.39, 0.38]),
                ('256 B decode', [6.27, 7.58, 6.93], [3.26, 3.39, 3.27], [1.17, 1.22, 1.23], [0.74, 0.75, 0.72]),
                ('4 KiB encode', [25.51, 18.76, 1.92], [20.55, 15.76, 2.46], [12.23, 8.99, 2.27], [0.46, 0.45, 0.39]),
                ('4 KiB decode', [20.33, 19.66, 2.60], [16.03, 15.50, 11.21], [7.74, 7.62, 6.66], [1.07, 1.06, 1.05]),
            )
        ),
    ),
    Chart(
