...
fn version_continues_after_padding(major: u8, minor: u8, patch: u8) -> bool {
    match (major, minor) {
        (3, 13) => patch >= 13,
        (3, 14) => patch >= 4,
...
mod tests {
    use super::{
        ADVANCED_STAGING_CAPACITY, AdvancedDecoder, LenientDecodeError, StrictSpecials,
        Translation, alphanumeric_prefix_scalar, decode_advanced_staging, decode_lenient_to_ptr,
        decoded_symbol_len, finish_advanced_staging, is_lenient_symbol, lenient_decode_table,
        lenient_decoded_len, lenient_symbol_count, stage_advanced_symbols, stage_advanced_value,
        translate_bytes_scalar, try_decode_advanced_staging, validate_advanced_staging,
        version_continues_after_padding,
    };
    ...
    fn x86_prefix_and_translation_kernels_match_scalar() {
        ...
        assert_eq!(
            unsafe { super::lenient_count_x86::alphanumeric_prefix_sse2(&valid) },
            97
        );
        assert_eq!(unsafe { super::alphanumeric_prefix_sse2(&valid) }, 97);
        let mut interrupted = valid.clone();
        interrupted[47] = b'!';
        assert_eq!(
            unsafe { super::lenient_count_x86::alphanumeric_prefix_sse2(&interrupted) },
            47
        );
        assert_eq!(
            unsafe { super::lenient_count_x86::sse2(&interrupted, Some(*b"@#")) },
            interrupted
                .iter()
        ...
        unsafe {
            super::lenient_count_x86::translate_sse2(&mut translated, b'@', b'+', b'#', b'/')
        };
        assert_eq!(translated, expected);
        let mut translated = original;
        unsafe { super::translate_bytes_sse2(&mut translated, b'@', b'+', b'#', b'/') };
        assert_eq!(translated, expected);

        if std::is_x86_feature_detected!("avx2") {
            assert_eq!(
                unsafe { super::lenient_count_x86::alphanumeric_prefix_avx2(&interrupted) },
                47
            );
            assert_eq!(
                unsafe { super::lenient_count_x86::avx2(&interrupted, Some(*b"@#")) },
                interrupted
                    .iter()
                    .filter(|&&byte| is_lenient_symbol(byte, Some(*b"@#")))
                    .count()
            ...
            unsafe { translate_bytes_scalar(&mut expected, b'@', b'+', b'#', b'/') };
            unsafe {
                super::lenient_count_x86::translate_avx2(&mut translated, b'@', b'+', b'#', b'/')
            };
            assert_eq!(translated, expected);
        }
    }
