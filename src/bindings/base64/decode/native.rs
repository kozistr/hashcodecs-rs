...
use super::super::{STANDARD_ALPHABET, output_too_small};
...
use lenient::{
    AlphanumericPrefix, TranslateBytes, decoded_symbol_len, select_alphanumeric_prefix,
    select_translate_bytes,
};
...
pub(super) fn decode_advanced<'py>(
    py: Python<'py>,
    input: &BytesLike<'_, '_>,
...
pub(super) fn decode_advanced_into(
    py: Python<'_>,
    input: &BytesLike<'_, '_>,
...
mod tests {
    ...
    fn x86_prefix_and_translation_kernels_match_scalar() {
        ...
        assert_eq!(
            unsafe { super::lenient::lenient_count_x86::alphanumeric_prefix_sse2(&valid) },
            97
        );
        assert_eq!(
            unsafe { super::lenient::alphanumeric_prefix_sse2(&valid) },
            97
        );
        let mut interrupted = valid.clone();
        interrupted[47] = b'!';
        assert_eq!(
            unsafe { super::lenient::lenient_count_x86::alphanumeric_prefix_sse2(&interrupted) },
            47
        );
        assert_eq!(
            unsafe { super::lenient::lenient_count_x86::sse2(&interrupted, Some(*b"@#")) },
            interrupted
                .iter()
        ...
        unsafe {
            super::lenient::lenient_count_x86::translate_sse2(
                &mut translated,
                b'@',
                b'+',
                b'#',
        ...
        unsafe { super::lenient::translate_bytes_sse2(&mut translated, b'@', b'+', b'#', b'/') };
        assert_eq!(translated, expected);

        if std::is_x86_feature_detected!("avx2") {
            assert_eq!(
                unsafe {
                    super::lenient::lenient_count_x86::alphanumeric_prefix_avx2(&interrupted)
                },
                47
            );
            assert_eq!(
                unsafe { super::lenient::lenient_count_x86::avx2(&interrupted, Some(*b"@#")) },
                interrupted
                    .iter()
                    .filter(|&&byte| is_lenient_symbol(byte, Some(*b"@#")))
                    .count()
            ...
            unsafe { translate_bytes_scalar(&mut expected, b'@', b'+', b'#', b'/') };
            unsafe {
                super::lenient::lenient_count_x86::translate_avx2(
                    &mut translated,
                    b'@',
                    b'+',
                    b'#',
