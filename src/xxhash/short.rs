...
pub(super) fn xxh3_64_small(data: &[u8], seed: u64) -> u64 {
    let len = data.len();
    if len == 0 {
...
pub(super) fn xxh3_64_medium(data: &[u8], seed: u64) -> u64 {
    let len = data.len();
...
pub(super) fn xxh3_64_midsize(data: &[u8], seed: u64) -> u64 {
    let len = data.len();
    let mut acc = (len as u64).wrapping_mul(P64_1);
    for i in 0..8 {
