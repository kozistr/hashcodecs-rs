...
BASE64_SCHEMA = ROOT / 'src' / 'bindings' / 'base64' / 'schema.rs'
...
BASE64_METHOD_DOC_PATTERN = re.compile(
    r'(?P<prefix>register!\(\s*[A-Z0-9_]+,\s*cr(?P<hashes>\#*)")'
    r'(?P<body>.*?)(?P<suffix>"(?P=hashes)\s*\);)',
    re.DOTALL,
)
...
def replace_runtime_docs(
    ...
    *,
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        seen.add(name)
        return f'{match.group("prefix")}{runtime_doc(function, python_315=python_315)}{match.group("suffix")}'

    rendered = pattern.sub(replace, source)
    if seen != expected:
        raise ValueError(f'{path}: documented native methods {sorted(seen)!r} do not match {sorted(expected)!r}')
    return rendered


def base64_schema_names() -> list[str]:
    source = BASE64_SCHEMA.read_text(encoding='utf-8')
    names = re.findall(r'\bname:\s+c"([^"]+)"', source)
    names.extend(
        re.findall(
            r'fixed_batch_binding!\(\s*[A-Z0-9_]+,\s*[a-z0-9_]+,\s*c"([^"]+)"',
            source,
        )
    )
    return names
...
def render_method_file(module: str, path: Path, functions: dict[str, ast.FunctionDef]) -> str:
    ...
    if module == 'base64':
        method_names = base64_schema_names()
        documentation_pattern = BASE64_METHOD_DOC_PATTERN
    else:
        method_names = re.findall(r'ml_name:\s+c"([^"]+)"', source)
        documentation_pattern = METHOD_DOC_PATTERN
